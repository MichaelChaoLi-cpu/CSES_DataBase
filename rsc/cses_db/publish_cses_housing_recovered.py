#!/usr/bin/env python3
"""Publish recovered 2007 lookup and 2013 questionnaire housing evidence additively."""
...
from extract_cses_housing_recovered_evidence import EVIDENCE, check_options, checked_evidence
...
SELF = "rsc/cses_db/publish_cses_housing_recovered.py"
VIEWS = ("cses_housing_value_dictionary_v3", "cses_housing_categories_v3")
...
COUNTS = {"alignment_releases": 1, "variable_mappings": 6, "value_mappings": 39, "load_runs": 1,
          "instruments": 4, "questions": 3, "source_variable_link_updates": 3}
...
LINK_TARGET_SQL = """SELECT sv.source_variable_id FROM cses_alignment.cses_source_variable sv
  ...
  WHERE s.survey_wave='2013' AND a.relative_path='data/raw/CSES2013.zip'
    AND d.member_path='CSES2013/CSES2013/CSES 2013.zip' AND d.nested_member_path='HHHousing.dta'
    AND sv.variable_name IN ('Q04_24','Q04_22A','Q04_07')"""
...
def source_links(connection):
    ...
    require(len(rows) == 3 and {r["variable_name"] for r in rows} == {"Q04_24", "Q04_22A", "Q04_07"},
            "Exact three housing link targets required")
    return rows
...
def provenance_desired(plan):
    ...
    for source in plan["evidence"]["sources"]:
        ...
        for q in source.get("questions", []):
            skips = [f"{o['code']}: {skip}" for o in q["options"] for skip in o["skip_instructions"]]
            questions.append({"source_file": source["source_file"], "question_code": q["question_code"],
                ...
                "response_options": q["options"], "skip_instruction": "; ".join(skips) or None,
                "question_grain": "household-wave", "repeat_context": {
                    "source_sheet": "04 Housing", "locators": q["locators"],
                    "transcription": "Whitespace/apostrophe-normalized original spreadsheet cell text; raw cells in release evidence"},
...
def validate_provenance(connection, plan):
    ...
    questionnaire = next(s for s in plan["evidence"]["sources"] if s["survey_wave"] == "2013")
...
def local_plan(root):
    ...
    require(spec["release_id"] == RELEASE and spec["target_waves"] == ["2007", "2013"]
            and spec["new_views"] == list(VIEWS) and spec["fields"] == list(FIELDS.values()), "Recovery scope changed")
    ...
    vocabulary = {(r["canonical_name"], r["approved_canonical_value"]): r["approved_canonical_label"]
                  for r in approved["approved_rows"]}
    review = json.loads((root / REVIEW).read_text())
    local = pd.read_parquet(root / HO).rename(columns=snake_case)
    rows, coverage, sources = [], [], {}
    for wave in spec["target_waves"]:
        p = next(p for p in review["profiles"] if p["survey_wave"] == wave)
        archive_sha = next(r["evidence"]["archive_sha256"] for r in review["code_rows"] if r["survey_wave"] == wave)
        ...
        require(hashlib.sha256(payload).hexdigest() == p["member_sha256"], "Housing member changed")
        with pd.io.stata.StataReader(io.BytesIO(payload), convert_categoricals=False) as reader:
            require(not reader.value_labels(), "Unexpected embedded housing value labels")
            raw = reader.read()
        ...
        target = local.loc[local.survey_wave.eq(wave)].reset_index(drop=True)
        expected_rows = spec["expected_rows"][wave]
        row_ids = target.source_row_id.str.rsplit(":", n=1).str[-1].astype(int)
        require(len(raw) == len(target) == expected_rows and sorted(row_ids) == list(range(1, expected_rows + 1)),
                "Raw-to-local rows not bijective")
        require(target.source_archive.map(source_archive).eq(sources[wave]["archive"]).all()
                and target.source_submodule.eq("::".join(chain)).all(), "Source identity differs")
        raw = raw.iloc[row_ids.to_numpy() - 1].reset_index(drop=True)
        for field in FIELDS.values():
            p = next(p for p in review["profiles"] if p["survey_wave"] == wave and p["canonical_name"] == field)
            pd.testing.assert_series_equal(raw[p["source_variable"]].astype("Float64"),
                                           target[field].astype("Float64"), check_names=False)
            instrument = next(s for s in evidence["sources"] if s["survey_wave"] == wave
                              and (wave == "2013" or s["field"] == field))
            question = next(q for q in instrument["questions"] if q["field"] == field) if wave == "2013" else None
            options = question["options"] if question else instrument["options"]
            definition = spec["definitions"][wave][field]
            check_options(options, definition["expected_labels"])
            require(len(options) == len(definition["expected_labels"]), "Duplicate evidence options")
            observed = {normalized_code(k): int(v) for k, v in target[field].value_counts().items()}
            require(set(observed) <= {o["code"] for o in options}, "Observed code absent from recovered evidence")
            nulls = int(target[field].isna().sum())
            require(nulls == p["current_sql_null_count"], "Null count changed")
            coverage.append({"survey_wave": wave, "field": field, "rows": expected_rows,
                             "nonnull": expected_rows - nulls, "nulls": nulls, "observed_codes": observed})
            for option in options:
                category = definition["canonical_categories"][option["code"]]
                rows.append({"survey_wave": wave, "field": field, "source_variable": p["source_variable"],
                             "source_code": option["code"], "category": category, "label": vocabulary[field, category],
                             "source_label": None, "observed_count": observed.get(option["code"], 0),
                             "evidence": {"approval_basis": spec["approval_basis"], "policy": spec["policy"],
                                 "evidence_type": instrument["instrument_type"], "source_file": instrument["source_file"],
                                 "source_sha256": instrument["source_sha256"],
                                 "source_archive_sha256": instrument["archive_sha256"], "option": option,
                                 "question": question, "housing_source": sources[wave],
                                 "embedded_housing_value_labels": False, "household_questionnaire_recovered": wave == "2013"}})
    require(len(rows) == spec["expected_dictionary_entries"] == 39
            and sum(r["observed_count"] > 0 for r in rows) == spec["expected_observed_code_combinations"] == 34
            and sum(r["observed_count"] for r in rows) == spec["expected_nonnull_values"] == 22289,
            "Dictionary scope changed")
    return {"release_id": RELEASE, "spec_sha256": sha256_file(root / SPEC), "rows": rows,
            "sources": sources, "coverage": coverage, "evidence": evidence, "raw_codes_unchanged": True}
...
def predecessors(connection):
    rows = connection.execute("""SELECT m.variable_mapping_id,m.dataset_id,m.canonical_variable_id,
      ...
      JOIN cses_meta.cses_source_archive a USING(source_archive_id)
      JOIN cses_meta.cses_survey s ON s.survey_id=d.survey_id
      WHERE s.survey_wave IN ('2007','2013') AND c.target_table='final_HO_CSES' AND c.canonical_name=ANY(%s)
        AND r.mapping_version<>%s ORDER BY s.survey_wave,c.canonical_name""", (list(FIELDS.values()), RELEASE)).fetchall()
    require(len(rows) == 6 and all(r["mapping_version"] == "cses-variable-catalog-v1" for r in rows),
...
def protected(connection, root):
    ...
    # Exclude only new natural keys; mask only the three authorized link columns.
...
def dictionary_query(plan):
    payload = [{"survey_wave": r["survey_wave"], "field": r["field"], "source_code": r["source_code"], "evidence": r["evidence"]}
               for r in plan["rows"]]
    return sql.SQL("""SELECT * FROM cses_analysis.cses_housing_value_dictionary_v2 UNION ALL
      ...
      JOIN cses_meta.cses_survey s ON s.survey_id=d.survey_id
      JOIN cses_meta.cses_source_archive a USING(source_archive_id)
      WHERE r.mapping_version={release} AND s.survey_wave=e.survey_wave AND s.survey_wave IN ('2007','2013') AND r.status='approved'
...
def view_queries(plan):
    ...
    text = inner.as_string().replace("'cses-housing-value-mapping-v1'::text AS housing_dictionary_version",
                                   "'cses-housing-interface-v3'::text AS housing_dictionary_version")
...
def desired(plan, manifest, digest):
    ...
    for p in manifest["predecessors"]:
        ...
        mappings.append({**{k: p[k] for k in fields}, "alignment_status": "approved",
                         ...
                         "User-approved recovered primary evidence: independent 2007 code lookup or 2013 questionnaire; "
                         "original numeric housing values and prior releases preserved."})
    ...
    return {"release": {"mapping_version": RELEASE, "status": "approved",
                        "description": "Recovered primary housing evidence for 2007 and 2013; 39 options, 6 rules, 4 instruments, 3 questions/links; raw data unchanged.",
            ...
            "run": {"run_scope": RELEASE, "source_manifest_sha256": canonical_sha256(plan),
                    ...
                    "validation_summary": {"execution_sha256": digest,
                        ...
                        "predecessors": manifest["predecessors"], "approval_basis": "user_approved_recovered_primary_evidence",
                        "plan_sha256": canonical_sha256(plan), "backup": manifest["backup"],
                        "physical_data_unchanged": True, "2007_household_questionnaire_recovered": False,
                        "2013_housing_questions_recovered": 3}}}
...
def validate_views(connection, root, plan):
    dictionary = connection.execute("SELECT * FROM cses_analysis.cses_housing_value_dictionary_v3 "
                                    "ORDER BY survey_wave,canonical_name,source_value").fetchall()
    require(len(dictionary) == 200, "Expected 161 retained plus 39 new dictionary rows")
    transferred = [r for r in dictionary if r["survey_wave"] in ("2007", "2013")]
    expected = {(r["survey_wave"], r["field"], r["source_code"]): r for r in plan["rows"]}
    require(len(transferred) == 39 and {(r["survey_wave"], r["canonical_name"], r["source_value"]) for r in transferred} == set(expected),
    ...
    unchanged = connection.execute("""SELECT count(*) AS n FROM (
      (SELECT * FROM cses_analysis.cses_housing_value_dictionary_v3 WHERE survey_wave NOT IN ('2007','2013')
       EXCEPT ALL SELECT * FROM cses_analysis.cses_housing_value_dictionary_v2)
      UNION ALL
      (SELECT * FROM cses_analysis.cses_housing_value_dictionary_v2 EXCEPT ALL
       SELECT * FROM cses_analysis.cses_housing_value_dictionary_v3 WHERE survey_wave NOT IN ('2007','2013'))) d""").fetchone()["n"]
    require(unchanged == 0, "Prior dictionary rows changed")
    rows = connection.execute("SELECT * FROM cses_analysis.cses_housing_categories_v3").fetchall()
    ...
    index = {(r["survey_wave"], r["source_archive"], r["source_submodule"], r["canonical_name"], r["source_value"]): r
             for r in dictionary}
    require(len(index) == 200, "Duplicate dictionary key")
    coverage = []
    all_wave_coverage = []
    for prefix, field in FIELDS.items():
        ...
        for r in rows:
            ...
            require(r["housing_dictionary_version"] == "cses-housing-interface-v3", "Interface version differs")
            by_wave.setdefault(r["survey_wave"], {"matched": 0, "source_null": 0, "unmapped_nonnull": 0})[status] += 1
            if r["survey_wave"] in ("2007", "2013"):
                counts[status] += 1
    ...
    return {"dictionary_rows": 200, "housing_rows": 77922, "housing_columns": 66,
            "recovered_years_combined_coverage": coverage, "physical_values_unchanged": True,
...
def prepare(root, backup_dir):
    ...
    from publish_cses_housing_2017 import validate as validate_v2
    validate_v2(root)
    ...
    for scope in ("metadata", "analysis_schema"):
        fd, name = tempfile.mkstemp(prefix=f"mda_cses_recovered_{scope}_", suffix=".dump", dir=backup_dir)
        os.close(fd)
        os.chmod(name, 0o600)
    ...
    paths = set(frozen_plan(root)["provenance"]["implementation_sha256"]) | {
        SELF, SPEC, HO, REVIEW, EVIDENCE, f"{DIRECTORY}/plan.json",
        "rsc/tests/test_cses_housing_recovered.py", "rsc/cses_db/extract_cses_housing_recovered_evidence.py",
        "rsc/cses_db/extract_cses_response_option_cells.py", "rsc/cses_db/publish_cses_housing_2017.py",
        "data/releases/cses-housing-2017-from-2016-v1/execution.json",
        "data/releases/cses-housing-2017-from-2016-v1/validation.json",
        "rsc/cses_db/publish_cses_housing_interface.py",
        "rsc/cses_db/publish_cses_value_mappings.py", "rsc/cses_db/correct_cses_housing_lighting.py",
        "rsc/cses_db/cses_lineage_graph.py", "data/lineage/cses_lineage_graph_v8.json"}
...
def apply(root, confirmation):
    ...
    with connect_database({"dbname": "mda"}) as c:
        ...
        if records(c) is None:
            ...
            for name, query in zip(VIEWS, view_queries(manifest["plan"]), strict=True):
                ...
                c.execute(sql.SQL("COMMENT ON VIEW cses_analysis.{} IS {}").format(sql.Identifier(name),
                    sql.Literal(f"{RELEASE}; execution {digest}; recovered 2007 code lookups and 2013 housing questionnaire; source data and v1/v2 retained.")))
        ...
        require(c.execute("SELECT count(*) AS n FROM cses_analysis.cses_housing_value_dictionary_v3").fetchone()["n"] == 200,
                "Dictionary reader access differs")
        require(c.execute("SELECT count(*) AS n FROM cses_analysis.cses_housing_categories_v3").fetchone()["n"] == 77922,
    ...
    print("published=True inserted_metadata_rows=54 updated_source_links=3 new_views=2 physical_data_unchanged=True", flush=True)
...
def export(root):
    ...
    prior = json.loads((root / "data/lineage/cses_lineage_graph_v8.json").read_text())
    with connect_database({"dbname": "mda"}) as c:
        read_only(c)
    ...
    for e in prior["edges"]:
        if e["type"] in {"schema_exposes_analysis_view", "schema_contains_metadata_relation", "relation_feeds_analysis_view", "dictionary_transferred_by_user_decision"}:
            builder.add_edge(e["type"], e["source"], e["target"], **e["properties"])
    for name in VIEWS:
    ...
    for n in graph["nodes"]:
        if n["type"] == "instrument" and n["properties"].get("source_file") in recovered:
            ...
            builder.add_edge("recovered_evidence_supports_release", n["id"], _node_id("alignment_release", RELEASE),
                             fields=[evidence["field"]] if "field" in evidence else list(FIELDS.values()),
    ...
    graph["source"]["housing_recovered_evidence"] = {"release_id": RELEASE, "execution_sha256": digest,
        "implementation_sha256": sha256_file(root / SELF), "prior_graph_sha256": sha256_file(root / "data/lineage/cses_lineage_graph_v8.json")}
    write_json(root / "data/lineage/cses_lineage_graph_v9.json", graph)
    write_json(root / "data/lineage/cses_housing_interface_topology_v3.json",
