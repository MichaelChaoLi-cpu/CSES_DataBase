#!/usr/bin/env python3
"""Import the exact reviewed CSES variable catalog in one transaction."""
...
IMPLEMENTATION_PATHS = (
    ...
    "rsc/cses_db/import_cses_variable_catalog.py",
    "rsc/cses_db/plan_cses_variable_catalog.py",
    "rsc/specs/cses_variable_catalog_v1.json",
)
...
def validate_reviewed_plan(
    ...
    if counts.get("alignment_releases") != 1 or counts.get("canonical_variables") != 280:
        raise ValueError("Reviewed plan does not contain the required release and 280 canonicals")
    if counts.get("source_variables", 0) <= 0 or counts.get("variable_mappings", 0) <= 0:
        raise ValueError("Reviewed plan has no source-variable catalog or reviewed mappings")
    if counts.get("load_runs") != 1:
        raise ValueError("Reviewed plan must contain exactly one load-run record")
    ...
    subprocess.run(["git", "merge-base", "--is-ancestor", planned_revision, "HEAD"], cwd=root, check=True)
...
def main() -> None:
    ...
    if not all(diagnostics["local_checks"].values()):
        ...
        raise SystemExit(f"Local variable catalog checks failed: {failed}")
    plan_path = args.plan or root / "data" / "processing" / "cses" / "variable_catalog_plan_v1.json"
    ...
    with connect_database(arguments) as connection:
        with connection.transaction():
            result = apply_variable_catalog(connection, desired, spec)

    output = args.output or root / "data" / "processing" / "cses" / "variable_catalog_import_v1.json"
