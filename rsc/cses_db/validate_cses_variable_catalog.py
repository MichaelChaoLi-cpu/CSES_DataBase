#!/usr/bin/env python3
"""Validate a completed CSES variable-catalog import without mutating mda."""
...
def build_validation_checks(
    ...
    return {
        ...
        "import_used_exact_reviewed_plan": import_evidence.get("reviewed_plan", {}).get("sha256")
        == plan_sha256,
        "import_code_revision_matches_plan": import_evidence.get("reviewed_plan", {}).get(
            "code_git_revision"
        )
        == plan["desired_state"]["load_runs"][0]["code_git_revision"],
        "all_reviewed_records_are_noops": database_preflight["action_counts"]
        == {"insert": 0, "noop": expected_noops, "conflict": 0},
        "all_database_checks_pass": all(database_preflight["checks"].values()),
        "import_inserted_expected_records": import_evidence["inserted_record_counts"]
        == plan["desired_record_counts"],
...
def git_revision(root: Path) -> str:
    result = subprocess.run(
        [
            "git", "log", "-1", "--format=%H", "--",
            "rsc/cses_db/validate_cses_variable_catalog.py",
        ],
        cwd=root,
        check=True,
        capture_output=True,
    ...
    if not revision:
        raise RuntimeError("Variable catalog validator is not committed in Git")
    return revision
...
def main() -> None:
    ...
    import_path = (
        ...
        or root / "data" / "processing" / "cses" / "variable_catalog_import_v1.json"
    )
    ...
    if desired != plan["desired_state"]:
        raise SystemExit("Current desired state differs from the reviewed variable catalog plan")
    ...
    report = {
        ...
        "import_evidence": {
            "path": str(import_path.relative_to(root)), "sha256": sha256_file(import_path)
        },
        "database_validation": database_preflight,
        "checks": checks,
        "validation_passed": all(checks.values()),
    }
    output = args.output or root / "data" / "processing" / "cses" / "variable_catalog_validation_v1.json"
    ...
    if not report["validation_passed"]:
        ...
        raise SystemExit(f"Variable catalog validation failed: {failed}")
